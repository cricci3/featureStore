...
f_source2 = FileSource(
    path=r"data/data_df2_2.parquet",
    event_timestamp_column="event_timestamp"
)

# Defining the second set of features
df2_fv = FeatureView(
    name="df2_2_feature_view",
    ttl=timedelta(days=2000),
    entities=[consumer_data],
    schema=[
...
target_source = FileSource(
    path=r"data/target_df_2.parquet",
    event_timestamp_column="event_timestamp"
)

# Defining the targets
target_fv = FeatureView(
    name="target_2_feature_view",
    entities=[consumer_data],
    ttl=timedelta(days=2000),
    schema=[
